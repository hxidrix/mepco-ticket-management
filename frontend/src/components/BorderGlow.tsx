...
function parseHSL(hslStr: string): { h: number; s: number; l: number } {
  ...
  const [, hue = '40', saturation = '80', lightness = '80'] = match;
  return { h: parseFloat(hue), s: parseFloat(saturation), l: parseFloat(lightness) };
}
...
function buildGlowVars(glowColor: string, intensity: number): Record<string, string> {
  ...
  for (let i = 0; i < opacities.length; i++) {
    ...
    vars[`--glow-color${keys[i] ?? ''}`] = `hsl(${base} / ${Math.min(opacity * intensity, 100)}%)`;
  }
  return vars;
}
...
function buildGradientVars(colors: string[]): Record<string, string> {
  const vars: Record<string, string> = {};
  for (let i = 0; i < 7; i++) {
    const colorIndex = Math.min(COLOR_MAP[i] ?? 0, Math.max(colors.length - 1, 0));
    const color = colors[colorIndex] ?? '#c084fc';
    const key = GRADIENT_KEYS[i] ?? '--gradient-one';
    const position = GRADIENT_POSITIONS[i] ?? '80% 55%';
    vars[key] = `radial-gradient(at ${position}, ${color} 0px, transparent 50%)`;
  }
  vars['--gradient-base'] = `linear-gradient(${colors[0] ?? '#c084fc'} 0 100%)`;
  return vars;
}
