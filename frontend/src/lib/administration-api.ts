...
export interface AuditItem { id:number;actorName:string|null;action:string;entityType:string;entityId:string|null;result:string;requestId:string|null;ipAddress:string|null;createdAt:string }
export interface StaffScope { id:number;userId:number;displayName:string;role:string;domain:'consumer'|'employee';departmentId:number|null;departmentName:string|null;categoryId:number|null;categoryName:string|null;circleId:number|null;circleName:string|null;canSelfAssign:number }
...
export async function auditRequest(page=1,search=''):Promise<{items:AuditItem[];meta:PaginationMeta}>{const response=await apiClient.get('/administration/audit',{params:{page,pageSize:30,...(search===''?{}:{search})}});return{items:unwrap<AuditItem[]>(response.data),meta:(response.data as {meta:PaginationMeta}).meta};}
