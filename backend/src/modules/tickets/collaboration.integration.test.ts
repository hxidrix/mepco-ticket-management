...
async function firstEmployeeTicket(accessToken: string): Promise<number> {
  ...
  if (id === undefined) throw new Error('Seed employee ticket missing');
  return id;
}
...
describe('ticket collaboration and notifications API', () => {
  it('keeps internal notes staff-only and delivers public comment notifications', async () => {
    ...
    const ticketId = await firstEmployeeTicket(employee);
