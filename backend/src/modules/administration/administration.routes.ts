...
administrationRouter.post('/announcements',authorizeRoles('supervisor','administrator'),...announcementRules,validateRequest,asyncHandler(async(request,response)=>{const id=await saveAnnouncement(request.auth!.id,null,request.body as {title:string;body:string;startsAt:string;endsAt?:string;isActive:boolean;audiences:UserRole[]},requestContext(request));sendSuccess(response,201,{id},'Announcement created');}));
administrationRouter.put('/announcements/:id',authorizeRoles('supervisor','administrator'),param('id').isInt({min:1}).toInt(),...announcementRules,validateRequest,asyncHandler(async(request,response)=>{await saveAnnouncement(request.auth!.id,Number(request.params.id),request.body as {title:string;body:string;startsAt:string;endsAt?:string;isActive:boolean;audiences:UserRole[]},requestContext(request));sendSuccess(response,200,null,'Announcement updated');}));
administrationRouter.delete('/announcements/:id',authorizeRoles('supervisor','administrator'),param('id').isInt({min:1}).toInt(),validateRequest,asyncHandler(async(request,response)=>{await deactivateAnnouncement(request.auth!.id,Number(request.params.id),requestContext(request));sendSuccess(response,200,null,'Announcement deactivated');}));
